...
int RunAttractMode(int curState, boolean curStateChanged) {
  ...
  if (attractPlayfieldPhase < 2) {
    ...
    for (byte count=0; count<15; count++) {
      if (count<=lampPhase1 && count>=(lampPhase1-4)) BSOS_SetLampState(LAMP_PLAYFIELD_SPELLOUT_S+count, 1);
      else BSOS_SetLampState(LAMP_PLAYFIELD_SPELLOUT_S+count, 0);
      if (count<=lampPhase2 && count>=(lampPhase2-3)) BSOS_SetLampState(LAMP_PLAYFIELD_STANDUP_S+count, 1);
      else BSOS_SetLampState(LAMP_PLAYFIELD_STANDUP_S+count, 0);
    }
  } else {
